...
chairs_path = '/PATH/TO/FlyingChairs_release/data'
chairs_split_file = '/PATH/TO/FlyingChairs_release/FlyingChairs_train_val.txt'
...
if args.checkpoint is None or args.clear_steps:
    ...
    run_id =  args.prefix + tag + logger.FileLog._localtime().strftime('%b%d-%H%M')

# initiate
from network import get_pipeline

pipe = get_pipeline(network=args.network, 
...
if args.checkpoint is not None:
    ...
    print('Network Class: ', network_class)

    # if train the head-stack network for the first time
    if network_class == 'MaskFlownet_DAM' and args.clear_steps and dataset_cfg.dataset.value == 'chairs':
    ...
    if network_class == 'MaskFlownet_DAM':
...
if dataset_cfg.dataset.value == 'kitti':
    ...
    orig_shape = dataset_cfg.orig_shape.get([384, 1216])
...
pipe.intialize_coords_pyramid(batch_size_card, target_shape)
