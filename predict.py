...
from flow_utils import read_flow, flow_to_image
...
def predict(pipe, prefix, batch_size = 8, resize = None):
    ...
    kitti_resize = (384, 1216) if resize is None else resize
    kitti_dataset = kitti.read_dataset_testing(editions='2015', resize = kitti_resize)
    prefix = prefix.replace('sintel', 'kitti')
    if not os.path.exists(prefix):
        os.mkdir(prefix)

    print('KITTI Resize: ', kitti_resize)

    for k, dataset in kitti_dataset.items():
        output_folder = os.path.join(prefix, 'flows_' + str(kitti_resize[0]))
        output_image_folder = os.path.join(prefix, 'flow_imgs_' + str(kitti_resize[0]))
        if not os.path.exists(output_folder):
            os.mkdir(output_folder)
        ...
        shapes = kitti_dataset[k]['shape']

        cnt = 0
        for result in pipe.predict(img1, img2, batch_size = 1, resize = kitti_resize):
            flow, disp_mask = result
            out_name = os.path.join(output_folder, '%06d_10.png' % cnt)
             
            shape = shapes[cnt]
            cnt = cnt + 1

            flow = cv2.resize(flow, (shape[1], shape[0]))
            flow[:, :, 0] = flow[:, :, 0] * (shape[1] / kitti_resize[1])
            flow[:, :, 1] = flow[:, :, 1] * (shape[0] / kitti_resize[0])
            ...
            disp_mask = np.array(disp_mask * 255).astype('uint8')
            disp_mask = cv2.cvtColor(disp_mask, cv2.COLOR_GRAY2BGR)
            cv2.imwrite(os.path.join(output_image_folder, '%06d_10_disp.png' % (cnt-1)), disp_mask)

            flow_img = flow_to_image(flow)
            cv2.imwrite(os.path.join(output_image_folder, '%06d_10.png' % (cnt-1)), flow_img)
